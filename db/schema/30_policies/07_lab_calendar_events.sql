...
CREATE POLICY "lab_calendar_lab_update" ON "public"."lab_calendar_events" AS PERMISSIVE FOR UPDATE TO "authenticated" USING ((has_org_role(lab_organization_id, ARRAY['Admin'::text, 'Manager'::text, 'Technician'::text]) AND ((owner_user_id = auth.uid()) OR ((calendar_scope = 'shared'::text) AND has_org_role(lab_organization_id, ARRAY['Admin'::text, 'Manager'::text]))))) WITH CHECK ((has_org_role(lab_organization_id, ARRAY['Admin'::text, 'Manager'::text, 'Technician'::text]) AND (calendar_scope = ANY (ARRAY['shared'::text, 'personal'::text])) AND ((owner_user_id = auth.uid()) OR ((calendar_scope = 'shared'::text) AND has_org_role(lab_organization_id, ARRAY['Admin'::text, 'Manager'::text])))));
