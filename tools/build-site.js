...
//     /demo/                                the demo, both languages in one page
//
...
const PAGES = [
  ...
  {
    // The demo is ONE page for both languages: it switches at runtime, since
    // a visitor mid-session must not be sent to another URL. So it is built
    // from its master with both languages kept and a button toggle instead
    // of links, and it stays out of the sitemap (it carries noindex).
    master: 'demo/index.html',
    bilingual: true,
    home: { ro: '/' },
    variants: { ro: { out: 'demo/index.html', url: ORIGIN + '/demo/' } },
    text: { ro: {} },
    // Its own assets, versioned by content like site.css.
    assets: ['/demo/demo.css', '/demo/demo.js'],
  },
];
...
function build(page, lang) {
  ...
  const button = (l) => '<button type="button" id="lang-' + l + '"' +
    (l === 'ro' ? ' class="active"' : '') + '>' + l.toUpperCase() + '</button>';
  const toggle = page.bilingual ? button : link;
  sub('<!--LANG-TOGGLE-->', toggle('ro') + EOL + '        ' + toggle('en'), 'lang toggle');
  ...
  // ── 5c. The page's own assets, by content hash, same as site.css.
  for (const asset of page.assets || []) {
    const file = path.join(OUT, asset);
    const v = version(fs.readFileSync(file, 'utf8'));
    sub('"' + asset + '"', '"' + asset + '?v=' + v + '"', asset + ' version');
  }
  ...
  console.log('  ' + page.variants[lang].out.padEnd(34) + (page.bilingual ? 'ro+en' : lang) +
    '  ' + (s.length / 1024).toFixed(0) + ' KB' + (page.bilingual ? '' : '  (' + drop + ' refs left: ' + left + ')'));
...
const entries = built.filter(({ page }) => !page.bilingual).map(({ url, page }) => {
  const alts = Object.entries(page.variants)
...
console.log('  sitemap.xml                       ' + built.filter(({ page }) => !page.bilingual).length + ' urls');
